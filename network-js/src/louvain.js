 ...
 * Finds communities in networks by optimizing modularity through hierarchical clustering.
 *
 ...
 * @class
 * @example
 * import { Louvain } from '@guinetik.network-js';
 *
 ...
 *   ])
 *   .execute();
 * console.log(communities); // { 'A': 0, 'B': 0, 'C': 1, 'D': 1 }
 */
export class Louvain {
  /**
   * Creates a new Louvain algorithm instance.
   */
  constructor() {
    /**
     * Maximum number of passes (-1 = unlimited)
     * @type {number}
     * @private
     */
    this.PASS_MAX = -1;
    ...
    /**
     * Minimum threshold for floating point comparisons
     * @type {number}
     * @private
     */
    this.MIN = 0.0000001;
    ...
    /**
     * Array of node identifiers in the original graph
    ...
    /**
     * Array of edges (Connection objects) in the original graph
     * @type {Connection[]}
     * @private
     */
    ...
    /**
     * Original graph representation with adjacency matrix
     * @type {Object}
     * @private
     */
    this.originalGraph = {};
     ...
     * @type {Object<string|number, number>|undefined}
     * @private
     */
   ...
   * Must be called before setEdges().
   *
   * @param {Array<string|number>} nodes - Array of unique node identifiers
   * @returns {Louvain} The Louvain instance for chaining
   * @example
   * louvain.setNodes(['node1', 'node2', 'node3']);
   */
  setNodes(nodes) {
   ...
   * Nodes must be set first via setNodes().
   *
   * @param {Array<Object|Connection>} edges - Array of edge objects with source, target, and optional weight
   * @param {string|number} edges[].source - Source node identifier
   * @param {string|number} edges[].target - Target node identifier
   * @param {number} [edges[].weight=1] - Edge weight (default: 1)
   * @returns {Louvain} The Louvain instance for chaining
   * @throws {Error} If nodes haven't been set first
   * @example
   * louvain.setEdges([
  ...
  /**
   * Set an initial partition for warm-start optimization.
   * Useful when you have prior knowledge of community structure.
   *
   * @param {Object<string|number, number>} partition - Mapping of node IDs to community IDs
   * @returns {Louvain} The Louvain instance for chaining
   * @example
   * // Start with a known partition
   * louvain.setPartitionInit({ 'A': 0, 'B': 0, 'C': 1, 'D': 1 });
   */
  ...
  /**
   * Execute the Louvain algorithm and return the final community partition.
   ...
   * 3. Communities are aggregated into super-nodes
   * 4. Repeat until modularity stops improving
   *
   * @returns {Object<string|number, number>} Mapping of node IDs to community IDs
   * @example
